    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <form onSubmit={addItem} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '6px' }}>
              <div className="field-group">
                ...
                <input
                  ...
                  placeholder="ex: aprovação do wireframe"
                  required
                />
              </div>
              <div className="field-group">
              ...
              <div className="field-group">
                ...
                <input
                  ...
                  placeholder="https://figma.com/... ou https://staging..."
                  type="url"
                />
              </div>
      ...
      {items.map(item => (
        <div key={item.id} className="approval-card">
          ...
          <div className="approval-card-header">
            ...
            {isAdmin && (
              <button
                ...
                style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'rgba(240,241,241,0.2)', background: 'none', border: 'none', cursor: 'pointer', padding: '0 0.25rem', flexShrink: 0 }}
                title="remover"
              >
                ✕
              </button>
          ...
          {/* link */}
          {item.link && (
            <a href={item.link} target="_blank" rel="noopener noreferrer" className="approval-link">
              ↗ ver entregável
            </a>
          )}

          {/* comentário do cliente (admin vê sempre, cliente vê o próprio) */}
          {item.client_comment && (
              ...
              {commenting === item.id ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginTop: '0.75rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                    <button className="btn-chassis" onClick={() => { setCommenting(null); setComment('') }} style={{ fontSize: '0.6rem', padding: '0.4rem 0.8rem' }}>
                      cancelar
                    </button>
                ...
                <div style={{ display: 'flex', gap: '0.6rem', marginTop: '0.75rem', flexWrap: 'wrap' }}>
                  <button
                    className="cta-btn"
                    onClick={() => respond(item.id, 'aprovado')}
                    style={{ flex: 1 }}
                  >
                    <span className="cta-led" />
                  ...
                  <button
                    className="btn-chassis"
                    onClick={() => { setCommenting(item.id); setComment('') }}
